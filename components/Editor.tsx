...
export default function Editor({
  ...
  const highlightErrors = useCallback((errorsToHighlight: ValidateError[]) => {
    ...
    const contentText = cm.getValue()
    const markers: any[] = []

    errorsToHighlight.forEach((error) => {
      if (error.startIndex >= 0 && error.endIndex <= contentText.length && error.startIndex < error.endIndex) {
        try {
          const marker = cm.markText(
            { ch: error.startIndex, line: 0 },
            { ch: error.endIndex, line: 0 },
  ...
  }, [getCMInstance, clearErrorHighlights])
  ...
  const handleApplySuggestion = useCallback((error: ValidateError) => {
    ...
    const contentText = cm.getValue()
    if (error.startIndex >= 0 && error.endIndex <= contentText.length) {
      const newContent = contentText.substring(0, error.startIndex) + error.suggestion + contentText.substring(error.endIndex)
      cm.setValue(newContent)
      onChangeContent(newContent)
      clearErrorHighlights()
    }
  }, [getCMInstance, onChangeContent, clearErrorHighlights])
  ...
  const jumpToError = useCallback((error: ValidateError) => {
    ...
    if (cm.charCoords && cm.scrollTo) {
      cm.scrollTo(null, cm.charCoords({ ch: error.startIndex, line: 0 }, 'local').top)
    }

    setTimeout(() => {
      highlightErrors(errors.filter(e => e.id !== error.id))
      setActiveErrorId(null)
    }, 2000)
  }, [getCMInstance, errors, highlightErrors])
